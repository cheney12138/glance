...
11. **通用页多一组「手势操控」**(2026-09-19,用户:「这次也做成开关吧」):两枚真开关,
    直读既有 defaults 键(键已存在,此前只能 `defaults write` 开 —— 病例:另一台机器装完
    0.3.2 三指四指"根本唤不起来",真凶就是 UserDefaults 每机独立、新机全默认关,而非权限):
     · **三指点按唤起面板**(key `pointer.threeFingerTapPanel`,默认关,对齐 macOS 无此功能);
     · **四指点按进未启动环**(key `pointer.fourFingerTapLaunchRing`,默认关,同上)。
    两键都是 UserDefaults 直读(改了立即生效,不重启)。开关**不重命名键** —— 已装机用户
    的现网值必须原样继承。
12. **导航 = 左侧纵向目录**(2026-09-19,参考 DockDoor):侧栏 176 + 内容区,窗宽 780。

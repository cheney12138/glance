...
| T59 | **第二份日志:逐元素投影被证实是热路径大头**;顺手修掉打卡器第二个 bug + 让实验开关自报状态 | `Diagnostics/SessionMarks.swift`(首图打卡用 `lastSessionStart` 而不是被 finish 清零的 `t0`)、`App/GlanceApp.swift`(开关开启时启动打一行) | 构建 ✅ | · **证据**:同样的外接屏 10–11 App,`[工] 键盘换选中` 从上一份的**每次 17–38ms(条条都打)**变成这一份**几乎一次都不打**(trace 阈值 2ms)⇒ 单次换选中 <2ms,**10–20×**;`[工] 指针换选中` 同样不再出现。剩余:`开窗` 10–43ms(冷启 146ms/首帧 80ms —— 闲置 ~10min 后第一局)。· **我自己的 bug**:修首图打卡时用了 `t0`,而 `finish()` 会把它清零 ⇒ 打出 `+811154268827ms`;另:实验开关的状态必须自己报(否则两份日志读半天不知道条件) |

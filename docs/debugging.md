...
## 20. 三指误触的最后一刀:拖移证据一票否决(2026-09-23)

**病例(实锤日志,`~/Library/Logs/Glance/trace.log` 22:43:50 会话)**:
```
[25180ms] 3 指 949ms 位移 norm=0.0855 size=0.8 → 不动作            ← 真拖移,判对了 ✓
[27900ms] 三指 237ms[state 4] 位移 norm=0.0251 abs=2.0 size=0.6 → 唤起 ✗  ← 误触
```
误触那发在**所有阈值上全部擦边通过**(237ms/300ms、0.0251/0.03、size 0.6/0.6),
而且位移 0.0251 **低于**同会有意轻点的 0.0287 ⇒ **任何静态阈值都分不开这一对** ✗。
它是"快速选词":三指拖移在**接触期间就完成了**,抬手后没有 dragged 事件 ⇒
事后撤销(`cancelIfDragStarted`)抓不到 ✗。

**修法(手指之外的证据)**:三指拖移被系统消费时必**合成 `leftMouseDown`**
("按住左键移动",文字能被选中的机制),真轻点不合成任何鼠标事件。
采集口 = 已有的 `mouseTap`(`.listenOnly` + leftMouseDown):**≥2 指在板时**见到
鼠标按下 ⇒ 记一笔证据;判卷时(滑动判定之后、生效之前)证据在 1.5s 窗口内 ⇒ 一票否决。

**日志口径**:被拦的局会出现一行
`[指点按] 按压期间出现过鼠标按下(三指拖移被系统消费的签名)⇒ 让给系统,不动作`
—— 它一出现就说明真拦住了一次误触 ✓;没有它 + 面板没弹 = 那轮本就不该生效(看其它"不动作"行)。
**已知代价**:真轻点唤起后 1.5s 内恰好有拖拽在进行 ⇒ 这次唤起被吞(再点一下即可,自纠正)。

**复现口径**:开 trace(`defaults write com.cheney12138.macswitcher debug.trace -bool true`
后重启 App,日志落 `~/Library/Logs/Glance/trace.log`),连续做 5 次快速选词 ⇒
日志里只应有 `鼠标按下…让给系统` 行,不应再出现 `→ 唤起(钉住)` ✗/✓ 对照。

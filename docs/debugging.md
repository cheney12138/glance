...
## 10. 「唤起即切换」开关到底生效没有(T34)

**别靠"高亮在第几格"判断** —— 开关开/关**两种状态都是高亮第一格**(用户口径:不要一唤起就选中
第二个),所以只看位置永远看不出开关有没有生效。每次开局现在都打一行:

```
[T6] 落点:唤起即切换=开 · 正向 · 选中 [1/6] 微信 · 顺序 [微信 | Xcode | WorkBuddy | 访达 | 预览 | Google Chrome]
```

看三件事:**开关状态**、**选中的是哪个 App**、**整条顺序**。判据:开关开着时,**当前 App 必须在
队尾**(左旋的结果),队首是"上一个 App"。

**最常见的误判**(我自己犯过一次):为了让高亮落在第一格而用 `swapAt(0,1)` —— 落点对了,但当前
App 被留在第二格,于是按一下 Tab 又绕回自己所在的地方,用户只会报一句"开关没生效"。
**正确的操作是左旋一格,不是交换两格**:交换只改两格的相对位置,旋转改的是整条环。
纯核规则 + 6 条用例在 `Packages/GlanceCore/LandingRule.swift`(`swift test` 钉着)。

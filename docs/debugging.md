...
## 16. 打卡器:一局的环节时间轴(2026-09-15)

用户口径:「还是有掉帧啊老师,不管是体感还是肉眼上。要不加一个你说的打卡器,看看各个环节?」

`traceCost` 只能量**一段**代码(还得先知道该量哪儿)✗ —— 掉帧最难的正是"不知道哪一段" ✗。
打卡器反过来:先把一局的每个环节都记下时间,让账自己浮出来。

**开**:`GLANCE_TRACE=1` 起来后,每次唤起多打两行 ——
```
[打卡] 唤起 共 42.1ms | 枚举 31.2 · 落点 0.1 · 开窗 3.8 · 首帧 6.9
[打卡] 首图上屏 +118ms(本批 10 张)
[T5] 预截 要拍 10 窗 → 回填 10 窗
```
平时(trace 关)只在**有环节超过 16.7ms** 时打一行,守住"一次唤起 ≤ 2 行"的预算。

**判读**:
- `枚举` 大 → 后台枚举本身慢(窗口多、AX 超时)—— 但它不在主线程,除非**回填**那一下卡;
- `开窗` 大 → `orderFrontRegardless` + 窗口后备存储分配(窗口越大越贵,随 App 数线性长);
- `首帧` 大 → SwiftUI 第一次渲染这棵树(图标数、卡片数、逐个阴影都在这里);
- `首图上屏` 晚 / `要拍 N 窗` 大 → 缩略图批次规模 —— **"外接屏 App 多就掉帧"这条体感首先怀疑它**
  (每扇窗一次抓屏是 GPU/WindowServer 上的活,与入场动画抢资源)。

**已埋的规模计数器**:`[T5] 预截 要拍 N 窗 → 回填 M 窗` 与 `[打卡] 首图上屏 +Nms(本批 M 张)` ——
前者说"这一局实际拍了几扇",后者说"这批图什么时候落到主线程"。

### 16.1 第一次实测(2026-09-15):`开窗` 与 `键盘换选中` 都随格子数增长

一份真机日志(内键屏 6 App / 外接屏 11 App 交替唤起,`GLANCE_TRACE=1`)的量值:

| 量 | 6 App(内键屏) | 11 App(外接屏) |
|---|---|---|
| `[打卡] … 开窗` | ≈ 17–27ms | ≈ **25–37ms** |
| `[工] 键盘换选中 主线程` | 6–22ms | **12–38ms** |
| `[帧] 长帧` | 0–2(0–4%) | 1–4(1–4%) |
| `[唤起] 按键→上屏` | 48–84ms | 38–119ms |

**两条真异常**:① `开窗` 每局 20–37ms(首局 92ms);② 每按一次 Tab 17–38ms。
两者都**随格子数增长** ⇒ 嫌疑锁定在"逐元素的重绘代价"。

**先排除掉的两个**:`[工] 日志开销:单行 0.07ms`(所以那 20ms 是真活 ✗ 不是打印);
`[尺寸] 的「玻璃 x/ypt」不是玻璃宽度`(是"按本局上限算的长条宽 / 可用宽",1511 = 2.36 × 641 对得上)——
所以窗口尺寸正常,不是"窗口过大"。

**头号嫌疑:SwiftUI 的逐元素 `.shadow(...)`** —— 它给**整棵子树**做一次离屏光栅化,
每个图标一次、每张卡片一次,正是"随元素数线性长"的形状。免编译的验证开关:
```bash
defaults write com.cheney12138.macswitcher debug.noElementShadows -bool true   # 关掉逐元素投影
defaults delete com.cheney12138.macswitcher debug.noElementShadows             # 开回来
```
重启 App 后对比 `[工] 键盘换选中` 与 `[打卡] 开窗`:**掉多少就是它的罪**。
（关掉后图标会"飘"起来 —— 那是诊断用的,不是设计。）

### 16.2 两条量具自身的局限(2026-09-15,我踩的)

1. **`traceCost` 只包住"状态写入"那一小段** —— 它**不包含**随后那次 SwiftUI 重绘。
   所以 `[工] 键盘换选中` 从 17–38ms 掉到 <2ms,只能说明**包住的那段**变快了,
   不能说明"面板画得更快了"。要量重绘,看 `[帧]` 与 `[打卡] 开窗`。
2. **实验开关必须自己报状态** —— 两份日志读半天,才发现我一直在推测"开关到底关没关",
   而 `defaults read` 显示它**从未被设置**。凡是"靠某个外部条件才成立的结论",
   要么让程序把它打出来,要么在日志里带上条件,否则等于没测。
...
## 18. 内建 ProMotion 屏的 `[帧]` 判读:均匀 16.7 不是卡,混拍才是(2026-09-16)

病例:用户报「⌘Tab 唤起后偶尔掉帧」。真实日志里内建 XDR(120Hz)的会话清一色
`长帧 100%(按 120Hz 判定)`,看着吓人 —— 但 P50 恒 **16.7ms**、毫无 8.3ms 的段落。

**结论(A/B 实测 + 一次意外发现)**:`debug.noGlass`(换 `NSVisualEffectView`)后内建屏照样
16.7ms P50 ⇒ 钉在 60Hz **与玻璃无关**。真正的相关变量是**显示器拓扑**:
"4K@60 外接屏 + 120Hz 内建屏"同时在线时,内建屏上面板内容节奏恒 16.7ms(20+ 局);
**外接屏断开后的同一块内建屏,同一份代码,P50 直接变成 8.3ms(真 120Hz)** ⇒
面板管线从来没有 60Hz 上限,钉住它与多屏混合刷新率下的系统合成有关(App 侧无解,
且均匀 16.7 在 60Hz 扫描下本来就读作"顺")。※ 拓扑相关性**已复现确认**(2026-09-16 同日):
外接屏接回的瞬间,内建屏 6/6 局立刻钉回 16.7ms;断开即 8.3ms(120Hz)。
同一份代码、同一 App 实例,唯一变量是拓扑 —— 记为**已知环境项**(App 侧无解;
均匀 16.7 在 60Hz 扫描下本来就读作"顺")。

肉眼可见的"卡"对应的是另一行 —— **节奏抖动**(±P90):混拍(8.3/16.7/25 交替)=
扫描节拍与内容节拍错位 = judder。判读口径:

| `[帧]` 读数 | 含义 |
|---|---|
| P50 16.7 均匀,无抖动行 | 顺(60Hz 节奏,别被 `长帧 100% [按 120Hz 判定]` 吓到) |
| 节奏抖动 ±>3ms | **卡**(混拍,这才是用户嘴里的"掉帧") |
| max 30–140ms 且时刻落在 +0.1–0.45s | 入场窗口里有主线程爆发(修法见 tasks.md T76/T79:其余组预截推迟 0.45s,显示组保持立刻重拍) |

**量具口径的另一条**:FrameProbe 挂在主线程,WindowServer/render server 侧的挤兑
(GPU 合成、backdrop blur)它看不见 —— "眼睛看到了但 `[帧]` 全绿" 才需要怀疑 GPU 侧,
验证用 Instruments 的 Core Animation 轨道。

## 19. 换环「抖一下」的真因:窗框宽度在换环那一刻跳变(2026-09-22)

**病例**:用户报「上下切换的时候,环有抖动。应该是变形导致的,直接 3/4 唤起打开是没问题的」。
一路消元(图标投影 ✗ / 长条投影 ✗ / 玻璃 ✗)都不对,最后是用户自己一句话点到根上:

> 「这不还是左右吗,从左边向右延伸出来的。这个动效不好的问题在于,**环是居中的,
>   2 个环其中一个比另一个短的话,左端的延伸位置就会变化,很割裂**」

**真因**:换环时窗框(=玻璃)宽度**当拍**换成新环的宽度(实测 **634 ↔ 1509**),
而环是**居中**的 ⇒ 左右两条边**同时移动** ⇒ 读成"从左边长出来 / 左端在变" ✗;
再加上当时窗框走 AppKit 动画、内容走 SwiftUI 动画 **两条时间线** ⇒ 相位差 = 肉眼里的抖 ✗。

**修法(两条,缺一不可)**:
1. **环窗口的宽度 = 一局的不变量** = 两环里更宽的那个(`PanelController.ringWindowWidth()`)
   —— 这是 ADR-0006「窗口尺寸是一局的不变量」补上"环窗口"这一块。
   环比窗框窄时,内容**居中**摆;玻璃仍按各自环宽画 ⇒ 换环时**左右边缘一动不动** ✓
2. **窗框侧不补间**(删掉 `NSAnimationContext` 那段)⇒ 只剩内容一个动画系统 ⇒
   双系统相位差**从构造上**不存在 ✓(不是"压小" ✓)

**判读口径(以后自测用)**:换环前后各打一次 `[窗框] 主面板 …`
—— 尺寸必须**逐字相同**(实测 `1383.9x374.4` ✓);只要它变了,就说明"宽度又按环算了" ✗。
一条命令即可复现:`swift Tools/SmokeRingCross.swift`(注入 HID 事件,与真键盘同构)。

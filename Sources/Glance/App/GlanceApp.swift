...
///
/// 所以这里的语义是**显式写死**的:
///   · 正向(⌘Tab):切到**最近用过的上一个 App** ⇒ 于是 A→B、再按 B→A ✓
///     (正是"在最近两个 App 之间来回"那条定义 ✓)
///   · 反向(⇧⌘Tab):切到 MRU 里**最久没用的那个** ✓(与系统 ⇧⌘Tab 同向 ✓)
/// 实现只做一件事:**激活那个 App**(与系统 ⌘Tab 一样是 App 级切换 ✓);
/// 不动窗口、不建面板、不枚举窗口 ⇒ 这也是它比"弹面板"快的原因 ✓
/// (窗口级/按屏归属的细腻做法在另一个功能里:接管 ⌘` ✓)
///
/// ⚠️ 为什么在 App 层:要用清点/落点那些模块 ⇒ 只有 App 层允许装配多模块 ✓
///   (与 `SameAppScreenCycler` 同一个理由,见那个文件的头注 ✓)
enum QuickSwitch {
    @MainActor
    static func toPreviousApp(forward: Bool) {
        let running = NSWorkspace.shared.runningApplications.filter { $0.activationPolicy == .regular }
        let order = MruEvidence.shared.ordered(pids: running.map(\.processIdentifier))
        guard order.count > 1 else {
            if isTraceEnabled { glog("[轻点⌘Tab] 运行中的 App 不足两个 ⇒ 不动作") }
            return
        }
        // 前台 App 在 MRU 里的位置(找不到就当成第一个 ✓)
        ...
        if isTraceEnabled {
            glog("[轻点⌘Tab] \(forward ? "正向" : "反向") ⇒ 激活 pid=\(target)"
                 + "(MRU 第 \(targetIdx + 1)/\(order.count) 位)")
        }
        NSRunningApplication(processIdentifier: target)?.activate(options: [])
    }
}
